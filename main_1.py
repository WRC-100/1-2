...
if __name__ == '__main__':
 ...
 ##导入数据集
    ...
    path = r"E:\python\data\2.xlsx"
    datamatrix = matrix(path)
    y_dmrs=datamatrix
    path = r"E:\python\data\1.xlsx"
    datamatrix = matrix(path)
    x_dmrs = datamatrix
    print(y_dmrs.shape)
    print(x_dmrs.shape)
...
## 归一化过程
    ...
    #
    #
    # pickle.dump(scaler, open("sca.dat", "wb"))  # open("dtr.dat","wb")意思是打开叫"dtr.dat"的文件,操作方式是写入二进制数据
...
# ****************************
 ...
 ##random_state{数据洗牌时的种子数}；
    ...
    clf_rbf = svm.SVC(decision_function_shape="ovo", kernel="rbf", C=5, gamma=0.0005)
    #clf_linear.fit(x_train,y_train)
    clf_rbf.fit(x_train, y_train)
    x_test_b = clf_rbf.decision_function(x_test)
    kk = clf_rbf.decision_function(x_train)
    clf_rbf_b = svm.SVC(decision_function_shape="ovo", kernel="rbf", C=5, gamma=0.0005)
    ...
    y_test_pre_rbf = clf_rbf_b.predict(x_test_b)

    ## 读出模型
    # pickle.dump(clf_rbf, open("dtr.dat", "wb"))  # open("dtr.dat","wb")意思是打开叫"dtr.dat"的文件,操作方式是写入二进制数据
